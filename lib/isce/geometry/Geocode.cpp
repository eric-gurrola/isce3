        ...
        isce::io::Raster & demRaster)
{
    ...
    for (size_t block = 0; block < nBlocks; ++block) {
        ...
        std::cout << "In function geocode: FirstLine, LastLine, FirstPix, LastPix " << std::endl
          << azimuthFirstLine << " "
                  << azimuthLastLine << " "
                  << rangeFirstPixel << " "
                  << rangeLastPixel << std::endl;
        ...
        for (size_t blockLine = 0; blockLine < geoBlockLength; ++blockLine) {
            ...
            for (size_t pixel = 0; pixel < _geoGridWidth; ++pixel) {
                ...
                rdrX = (srange - _radarGrid.startingRange())
                  / (_radarGrid.rangePixelSpacing() * _radarGrid.numberRangeLooks());
                        ...
                        int & rangeFirstPixel, int & rangeLastPixel)
{
    ...
    azimuthFirstLine = (azimuthTime.min() - _radarGrid.sensingStart()) * (_radarGrid.prf() / _radarGrid.numberAzimuthLooks());

    // the last azimuth line
    azimuthLastLine = (azimuthTime.max() - _radarGrid.sensingStart()) * (_radarGrid.prf() / _radarGrid.numberAzimuthLooks());
    ...
    std::cout << "In Function _computeRangeAzimuthBoundingBox: FirstLine, LastLine, FirstPix, LastPix " << std::endl
              << azimuthFirstLine << " "
              << azimuthLastLine << " "
              << rangeFirstPixel << " "
              << rangeLastPixel << std::endl;
